...
class Backend:
    def __init__(self):
        ...
        self.pulling_interval = int(self.config.get("PULLING_INTERVAL", 5))
        ...
        while True:
            ...
            if cat_mask is not None and confidence >= self.cat_tolerance:
                ...
                if iou > self.iou_tolerance:
                    logging.info(
                        f"{class_name.capitalize()} inside activation mask (IoU: {iou:.2f}), activating pump for {self.pump_duration}s..."
                    )
                    try:
                    ...
                    logging.info(
                        f"{class_name.capitalize()} not sufficiently inside activation mask (IoU: {iou:.2f})."
                    )
