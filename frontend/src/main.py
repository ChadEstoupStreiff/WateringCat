...
def main():
    ...
    if tab == "Monitor":
        ...
        cols[4].metric("Consecutive Required", status.get("consecutive_required", "N/A"))
        pump_cooldown = status.get("pump_cooldown")
        cols[5].metric("Pump Cooldown", f"{pump_cooldown}s" if pump_cooldown is not None else "N/A")
